...
debugger
  ...
  async getProblemsByEquipment(equipmentId: number, businessId: number): Promise<Problem[]> {
    return this.problemRepository.find({
      where: {
        equipment: { id: equipmentId }
      },
  ...
  private async findSimilarProblemsByType(
    ...
    const scoredProblems = problemsWithSolutions.map(problem => {
      ...
      if (problem.equipment?.manufacturer?.toLowerCase() === manufacturer.toLowerCase()) {
        score += 2;
      }
    ...
    scoredProblems.sort((a, b) => {
      ...
      return new Date(b.problem.createdAt).getTime() - new Date(a.problem.createdAt).getTime();
    });
