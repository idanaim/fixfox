...
# FixFox Production Deployment Script
# This script deploys to AWS App Runner PRODUCTION and performs health checks
...
echo -e "${BLUE}🚀 FixFox PRODUCTION Deployment Script${NC}"
echo "========================================"
...
# Get service ARN
echo "🔍 Looking for PRODUCTION App Runner service..."
...
if [ -z "$SERVICE_ARN" ] || [ "$SERVICE_ARN" = "None" ]; then
    echo -e "${RED}❌ PRODUCTION App Runner service '$SERVICE_NAME' not found.${NC}"
    echo ""
    echo "To create the PRODUCTION service first:"
    echo "1. Run: ./scripts/setup-app-runner-prod.sh"
    echo "2. Or follow: scripts/app-runner-setup-guide.md"
    exit 1
fi

echo -e "${GREEN}✅ PRODUCTION Service found${NC}"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${RED}❌ Failed to start PRODUCTION deployment${NC}"
    exit 1
fi
...
while [ $WAIT_TIME -lt $MAX_WAIT_TIME ]; do
    ...
    case $STATUS in
        ...
        "CREATE_FAILED"|"UPDATE_FAILED")
            echo -e "${RED}❌ PRODUCTION Deployment failed with status: $STATUS${NC}"
...
if [ $WAIT_TIME -ge $MAX_WAIT_TIME ]; then
    echo -e "${YELLOW}⚠️  PRODUCTION Deployment is taking longer than expected${NC}"
    echo "Check the App Runner console for status updates"
fi

echo ""
echo -e "${BLUE}📊 Getting PRODUCTION service information...${NC}"
...
if [ "$SERVICE_URL" = "null" ] || [ -z "$SERVICE_URL" ]; then
    ...
    echo -e "${GREEN}🌐 PRODUCTION Service URL: https://$SERVICE_URL${NC}"
    echo -e "${BLUE}🏥 PRODUCTION Health Check URL: https://$SERVICE_URL/api/health${NC}"
    
    # Perform health checks if service is running
    if [ "$SERVICE_STATUS" = "RUNNING" ]; then
        echo ""
        echo -e "${BLUE}🏥 Performing PRODUCTION health checks...${NC}"
        ...
        for i in $(seq 1 $HEALTH_CHECK_RETRIES); do
            ...
            if [ "$response" = "200" ]; then
                echo -e "${GREEN}✅ PRODUCTION Health check passed! (HTTP $response)${NC}"
                
                # Get detailed health info
                echo ""
                echo -e "${BLUE}📊 PRODUCTION Health Details:${NC}"
                health_data=$(curl -s --max-time 10 "https://$SERVICE_URL/api/health" 2>/dev/null || echo "Failed to get health data")
                echo "$health_data" | jq . 2>/dev/null || echo "$health_data"
                break
            else
                echo -e "${YELLOW}⚠️  PRODUCTION Health check attempt $i failed (HTTP $response)${NC}"
        ...
        if [ "$response" != "200" ]; then
            echo -e "${RED}❌ All PRODUCTION health check attempts failed${NC}"
...
if [ "$SERVICE_URL" != "null" ] && [ -n "$SERVICE_URL" ]; then
    echo "• PRODUCTION API Documentation: https://$SERVICE_URL/api/docs"
    echo "• PRODUCTION Health Check: https://$SERVICE_URL/api/health"
fi

echo ""
echo -e "${GREEN}🎉 PRODUCTION Deployment process completed!${NC}"
echo "Your FixFox PRODUCTION API is now live and auto-scaling!" 